...
if len(sys.argv) not in [3,4] or sys.argv[2].upper() not in ['Y','N']:
	print
	print 'usage: <BLAST output> <extract fasta? (Y/N)> <SCORE cutoff> (OPTIONAL: default ALL hits)'
	...
	print '	 - hits aligned regions fasta file (original hits only), only if "extract fasta" = Y (yes)'
