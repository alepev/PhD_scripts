...
for R in Rsort:
	...
	for i in range(SCAN,len(Gsort)):
		gene=Gsort[i]
		chr2,start2,stop2=gene[0],gene[1],gene[2]
		print R,gene,
		if chr!=chr2:
			print
			SCAN=i
			break
		elif (start<=start2 and stop>=start2) or (start>=start2 and start<=stop2):
			if FIRST==True:
				SCAN=i
			FIRST=False
			...
			print '\tX!'
		elif stop<start2: #RNA ends before gene, meaningless to continue
			print
			break
